...
binance_ws_spot_socket <- function(pair = "BTCUSDT", subscription = "kline", interval = "1m", update_speed = 1000, nobs_max = NULL, quiet = FALSE){
  ...
  ws$onOpen(function(event) {
    ...
    # verbose message 
    if (!quiet) {
      msg <-  paste0("Stream: ", pair_name, "@", subscription, " opened!")
      message(msg)
    }
  })
  ...
  ws$onClose(function(event) {
    ...
    # verbose message 
    if (!quiet) {
      msg <- paste0("ERROR: Stream: ", pair_name, "@", subscription, " failed to connect: ", event$code, "\n")
      message(msg)
    }
  })
...
binance_ws_spot_url <- function(pair = "BTCUSDT", subscription = "kline", interval = "1m", update_speed = 1000, quiet = FALSE){
  ...
  if (subscription == "kline") {
    ...
    # websocket url
    websocket_url <- paste0(base_url, "/", pair_name, "@", subscription, "_", interval)
    # verbose message
    msg <- paste0('The stream is klines: ', pair_name, "@", subscription, "_", interval)
    ...
    websocket_url <- paste0(base_url, "/", pair_name, "@", subscription, "_", interval)
    # verbose message
    msg <- paste0('The stream is a ticker with Window Size: ', pair_name, "@", subscription, "_", interval)
    ...
    # websocket url
    websocket_url <- paste0(base_url, "/", pair_name, "@", subscription, "@", update_speed, "ms")
    # verbose message
    msg <- paste0('The stream is Depth with Update Speed: ', pair_name, "@", subscription, "@", update_speed, "ms")
    
  } else {
    # websocket url
    websocket_url <- paste0(base_url, "/", pair_name, "@", subscription)
    # verbose message
    msg <- paste0('The stream is: ', pair_name, "@", subscription, "@", update_speed, "ms")
  }
...
binance_ws_fapi_socket <- function(pair = "BTCUSDT", subscription = "kline", interval = "1m", contract_type = NULL, update_speed = 1000, nobs_max = NULL, quiet = FALSE){
  ...
  websocket_url <- binance_ws_fapi_url(pair = pair_name, subscription = subscription, interval = interval, contract_type = contract_type, update_speed = update_speed, quiet = quiet)
...
binance_ws_fapi_url <- function(pair = "BTCUSDT", subscription = "kline", interval = "1m", contract_type = NULL, update_speed = 1000, quiet = FALSE){
  ...
  if (subscription %in% c("kline", "continuousKline")) {
    ...
    # websocket url
    websocket_url <- paste0(base_url, "/", pair_name, contract_type, "@", subscription, "_", interval)
    # verbose message
    msg <- paste0('The stream is klines: ', pair_name, "(", contract_type, ")", "@", subscription, "_", interval)
    ...
    websocket_url <- paste0(base_url, "/", pair_name, "@", subscription)
    # verbose message
    msg <- paste0('The stream is: ', pair_name, "@", subscription, "@")
  }
