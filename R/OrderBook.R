...
OrderBook <- function(data = NULL, min_price = NULL, max_price = NULL, levels = 20, trades = NULL, as_datatable = FALSE){
  ...
  if (as_datatable) {
    ...
    order_book <- DT::formatRound(order_book, c("bid", "ask", "price"), 2)
    # format for BIDs
    order_book <- DT::formatStyle(order_book, "bid",
                                 background = DT::styleColorBar(range(order_book$bid), 'green'),
                                 backgroundSize = '98% 88%',
                                 backgroundRepeat = 'no-repeat',
                                 backgroundPosition = 'center')
    order_book <- DT::formatStyle(order_book, 'bid', backgroundColor = "#00E679", color = "white")
    # format for ASKs 
    order_book <-DT::formatStyle(order_book, "ask",
