...
source("R/binance_api.R")
source("R/binance_ws.R")
source("R/binance_depth.R")
source("R/binance_ws_cleaner.R")
...
ws$onMessage(function(event) {
  ...
  stream[[length(stream) + 1]] <- new_data # binance_ws_cleaner.depth(new_data)
  ws$.__enclos_env__[["stream"]] <- stream
})
...
data <- ws$.__enclos_env__$stream[[1]]

binance_ws_cleaner.depth(ws$.__enclos_env__$stream[[1]])

ws$.__enclos_env__$stream[[1]]
#ws$close()
